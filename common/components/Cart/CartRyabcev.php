...
class CartRyabcev extends Component
{
    const CONST_ID_CART = 'cart_strongman';

    const GOOD_MODEL = 'common\models\goods\Good';
    ...
    public function add( $id, $count, $size )
    {
        ...
        $obj = new self::GOOD_MODEL;


        if ( ! empty( $model ) ) {
            $this->setGoods( $id, (int) $count, $model );
            return true;
        }
        return false;
    }
    ...
    */
    public function setGoods( $id, $count, $model )
    {
        $this->goodsClass="common\models\goods\Good";
        ...
        if ( $this->hasGoods( $id ) ) {
            ...
            $cart[self::CATALOGUE_KEY_CART][$id]['totalPrice'] = $cart[self::CATALOGUE_KEY_CART][$id]['count'] * $model->price;
    ...
    public function delete( $id )
    {
        ...
        if ( $this->hasGoods( $id ) ) {
            $cart['count'] = $cart['count'] - $cart[self::CATALOGUE_KEY_CART][$id]['count'];
            $cart['countModel'] -= 1;
